...
/**
 * The Roostr mark at the DAG's growing tip: a circular badge pinned
 * where the newest changes land, sharing the scene's parallax.
 */
export const HeroLogo = shader({
	...
	vertex({ aCorner }, { uViewProj, uMouse }, v) {
		...
		const ct = cos(tilt);
		const st = sin(tilt);
		// At the tip: right side, slightly forward.
		const cx = 2.55 + aCorner.x * 0.26;
		const px = cx * cy + 0.3 * sy;
		const pz0 = 0.3 * cy - cx * sy;
