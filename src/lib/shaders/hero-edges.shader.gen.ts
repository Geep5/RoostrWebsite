...
const HeroEdges: CompiledShader<{ aQuad: 'vec2' }, { iStart: 'vec3'; iEnd: 'vec3'; iTint: 'vec3' }, { uViewProj: 'mat4'; uMouse: 'vec2'; uWidth: 'float' }> = {
...
fn vs_main(bm_in : BmVSIn) -> BmVSOut {
  ...
  let b = vec3f(bx, by, bz1);
  let dir = b - a;
  let len = length(dir);
  let n = dir * (1.0 / max(len, 0.0001));
  let perp = vec3f(-n.y, n.x, 0.0);
  let p = a + n * (bm_in.aQuad.y * len) + perp * (bm_in.aQuad.x * bm_u.uWidth * 0.5);
