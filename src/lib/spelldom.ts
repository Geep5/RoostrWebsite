...
import { checkText, dictionaryLoaded, loadDictionary, RETRY_MS } from "$lib/spell";
...
export async function refreshSpell(root: HTMLElement): Promise<void> {
	...
	// Dictionary not available (offline, failed transfer): come back after the
...
/** The misspelled word at a screen point with its offsets in the editable's text, or null. */
export function misspelledAt(x: number, y: number): { word: string; el: HTMLElement; from: number; to: number } | null {
	...
	const text = spans.map((s) => s.node.data).join("");
	for (const miss of checkText(text)) {
		if (global >= miss.from && global <= miss.to) return { word: miss.word, el, from: miss.from, to: miss.to };
	}
	return null;
}
