...
function rangeFor(spans: NodeSpan[], from: number, to: number): Range | null {
  ...
  for (const s of spans) {
    ...
    if (!a && from >= s.start && from <= end)
      a = { node: s.node, off: from - s.start };
...
export function misspelledAt(
  x: number,
  y: number,
): { word: string; el: HTMLElement; from: number; to: number } | null {
  const caret = document.caretRangeFromPoint?.(x, y);
  if (!caret) return null;
  const node = caret.startContainer;
  if (node.nodeType !== Node.TEXT_NODE) return null;
  const el = (node.parentElement as HTMLElement)?.closest<HTMLElement>(
    "[contenteditable]",
  );
  if (!el) return null;
  const spans = textNodes(el);
  ...
  const text = spans.map((s) => s.node.data).join("");
  for (const miss of checkText(text)) {
    if (global >= miss.from && global <= miss.to)
      return { word: miss.word, el, from: miss.from, to: miss.to };
  }
  return null;
}
