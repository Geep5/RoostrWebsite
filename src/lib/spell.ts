...
/** True once the word lists are in memory. */
export function dictionaryLoaded(): boolean {
	return dict !== null;
}

/**
 * Loads the word lists once. A failed or partial load (offline, a dropped
 * 2.5 MB transfer on cellular, an old service worker) leaves `dict` null,
 * so nothing is flagged and a later spell pass retries - an empty
 * dictionary would underline every word in the document.
 */
export function loadDictionary(): Promise<void> {
	if (dict || Date.now() < retryAt) return Promise.resolve();
	loading ??= Promise.all([fetchList("/dict-en.txt"), fetchList("/dict-en-common.txt")])
		.then(([headwords, common]) => {
			...
			// A plausible list has six figures of entries; anything smaller is a wrong response body.
			if (s.size < 100_000) throw new Error(`dictionary has ${s.size} words`);
		...
		.catch((err: unknown) => {
			...
			console.warn("spellcheck dictionary unavailable; retrying later", err);
		})
		.finally(() => {
			loading = null;
		});
